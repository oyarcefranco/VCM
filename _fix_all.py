...
HTML_MAIN = r"c:\Users\oyarc\OneDrive - Universidad San Sebastian\VCM\Encuestas Estudiantes\dashboard_vcm.html"
HTML_DETALLE = r"c:\Users\oyarc\OneDrive - Universidad San Sebastian\VCM\Encuestas Estudiantes\dashboard_vcm_detalle.html"

# Common UI for sec-dim
NEW_SEC_DIM = '''
    <div id="sec-dim" class="section hidden">
      ...
      <div class="year-filter" id="yf-dim">
        <button class="year-btn active" onclick="setDimYear(null)">Todos</button>
        <button class="year-btn" onclick="setDimYear('2021')">2021</button>
        <button class="year-btn" onclick="setDimYear('2022')">2022</button>
        <button class="year-btn" onclick="setDimYear('2023')">2023</button>
        <button class="year-btn" onclick="setDimYear('2024')">2024</button>
        <button class="year-btn" onclick="setDimYear('2025')">2025</button>
      </div>
      <div id="dim-content"></div>
    </div>
'''

# We need to extract DIM_DATA from dashboard_vcm.html to inject it into detalle
with open(HTML_MAIN, 'r', encoding='utf-8') as f:
    main_html = f.read()

dim_data_match = re.search(r'var DIM_DATA = (\{.*?\});\s*var dimState', main_html, re.DOTALL)
if not dim_data_match:
    print("Could not find DIM_DATA in main HTML")
    exit(1)
dim_data_str = dim_data_match.group(1)

NEW_JS = f'''
    var DIM_DATA = {dim_data_str};
    ...
    function setDimYear(y) {{
      dimYear = y;
      var btns = document.getElementById('yf-dim').querySelectorAll('.year-btn');
...
# 1. Update dashboard_vcm.html
main_html = re.sub(r'<div id="sec-dim".*?<div id="dim-content"></div>\s*</div>', NEW_SEC_DIM.strip(), main_html, flags=re.DOTALL)
main_html = re.sub(r'var DIM_DATA = \{.*?(?=function buildTrends)', NEW_JS, main_html, flags=re.DOTALL)
with open(HTML_MAIN, 'w', encoding='utf-8') as f:
    f.write(main_html)

# 2. Update dashboard_vcm_detalle.html
with open(HTML_DETALLE, 'r', encoding='utf-8') as f:
    det_html = f.read()

# Replace the nav link
det_html = re.sub(
    r'<a href="dashboard_vcm_dim5\.html"[^>]*>.*?</a>',
    r'<button class="nav-btn" onclick="showSection(\'dim\')" style="background: #e84393; color: #fff;">Modelo de Evaluación VcM</button>',
    det_html
)

# Insert the sec-dim block before sec-trend
if '<div id="sec-dim"' not in det_html:
    det_html = det_html.replace('<div id="sec-trend"', NEW_SEC_DIM.strip() + '\n    <div id="sec-trend"')

# Insert the JS before buildTrends()
if 'var DIM_DATA =' not in det_html:
    det_html = re.sub(r'(function buildTrends\(\) \{)', NEW_JS + r'\n    \1', det_html)

# Add buildDimensions(); to init()
if 'buildDimensions();' not in det_html:
    det_html = re.sub(r'(function init\(\) \{.*?)(^\s*\})', r'\1      buildDimensions();\n\2', det_html, flags=re.DOTALL | re.MULTILINE)

with open(HTML_DETALLE, 'w', encoding='utf-8') as f:
    f.write(det_html)
